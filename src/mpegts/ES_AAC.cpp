...
void ES_AAC::Parse(STREAM_PKT* pkt) {
  // TSDemux::STREAM_PKT *tmp_pkt = new TSDemux::STREAM_PKT;
  // Parse2(tmp_pkt);
  AP4_AacFrame frame;
  AP4_Result result = adts_parser.FindFrame(frame);
  if (result == AP4_SUCCESS) {
    frame.m_Info.m_FrameLength += 7;
    frame.m_Source->SkipBytes(-7);
    ...
    pkt->pid            = pid;
    pkt->data           = sample_data.UseData();
    pkt->size           = frame.m_Info.m_FrameLength;
    pkt->duration       = 1024 * 90000 / (frame.m_Info.m_SamplingFrequency);
    // pkt->duration = (c_pcr - p_pcr) / 300ULL;
    pkt->dts            = c_dts;
    pkt->pts            = c_pts;
    pkt->pcr            = c_pcr;
    bool streamChange = SetAudioInformation(frame.m_Info.m_ChannelConfiguration,
        frame.m_Info.m_SamplingFrequency, 0, 0, 0);
    pkt->streamChange   = streamChange;

    c_pts += 90000 * 1024 / (frame.m_Info.m_SamplingFrequency);
  }
}

void ES_AAC::Parse2(STREAM_PKT* pkt)
{
  int p = es_parsed;
  int l;
  while ((l = es_len - p) > 8)
...
int ES_AAC::FindHeaders(uint8_t *buf, int buf_size)
{
  ...
  else if (stream_type == STREAM_TYPE_AUDIO_AAC_ADTS)
  {
    if(buf_ptr[0] == 0xFF && (buf_ptr[1] & 0xF0) == 0xF0)
    {
      ...
      m_DTS = c_pts;
      m_PTS = c_pts;
      c_pts += 1024 * 90000 / (!m_SampleRate ? aac_sample_rates[4] : m_SampleRate);
      return -1;
    }
  }
  return 0;
...
int ES_AAC::Append(const unsigned char* buf, size_t len, bool new_pts) {
  ElementaryStream::Append(buf, len, new_pts);
  AP4_Size size = static_cast<AP4_Size>(len);
  if (adts_parser.Feed(buf, &size, 0) != AP4_SUCCESS) {
    std::cerr << "Unable to feed buffer\n";
  }
}
